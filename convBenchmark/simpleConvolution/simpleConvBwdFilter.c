...
void simple_convBwdFilter(convMess cm)
{
    fprintf(stdout, "\033\[31mSimple Convolution Backward for Filter ......\033\[0m\n");
    ...
    // kernel compute for convolution backward for filter
...
#endif
                    fltdiff[fltidx] = 0.0; // fltdiff初始化零
                    for(n = 0; n < N; n++){
                        for(oh = 0; oh < outH; oh++){
                            for(ow = 0; ow < outW; ow++){
                                ...
                                if((true_ih >= 0) && (true_ih < inH) && (true_iw >= 0) && (true_iw < inW)){
                                    ...
                                    fltdiff[fltidx] += in[inidx]*outdiff[outidx];
                                }
                            }
                        }
                    }
