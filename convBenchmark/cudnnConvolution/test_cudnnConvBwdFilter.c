...
int main(int argc, char *argv[]){
    ...
    FILE *pfoutput = fopen("./data/cudnn_convBwdFilter_nchw.txt", "r+");
    ...
    while(!feof(pfconfig)){
        ...
        fprintf(stdout, "CUDNN Convolution Message:\n");
        ...
        fltdiff = (DataType*)malloc(fltsize*sizeof(DataType));

        fileInit4dData(pfdata, in, indim); // file init data
        fileInit4dData(pfdata, outdiff, outdim);
        fileInit4dData(pfdata, fltdiff, fltdim);
        ...
        cudnn_convBwdFilter(cm); // cudnn convolution forward
        ...
        for(i = 0; i < K*C*fltH; i++){
            for(j = 0; j < fltW; j++){
                fprintf(pfoutput, "%lf ", fltdiff[i*fltW+j]);
            }
            fprintf(pfoutput, "\n");
        }
