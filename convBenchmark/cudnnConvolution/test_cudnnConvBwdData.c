...
int main(int argc, char *argv[]){
    ...
    FILE *pfoutput = fopen("./data/cudnn_convBwdData_nchw.txt", "r+");
    ...
    while(!feof(pfconfig)){
        ...
        fileInit4dData(pfdata, flt, fltdim); // file init data
        ...
        cudnn_convBwdData(cm); // cudnn convolution forward
        ...
        for(i = 0; i < N*C*inH; i++){
            for(j = 0; j < inW; j++){
                fprintf(pfoutput, "%lf ", indiff[i*inW+j]);
            }
            fprintf(pfoutput, "\n");
        }
